...
import {
  Shield, AlertTriangle, Zap, Database, TrendingUp, MessageSquare, Radio, Phone,
  CreditCard, Link2, Mail, Target, Wifi, WifiOff, BarChart3, PieChart, Send, Copy,
  RefreshCw, Bot, UserX, Fingerprint, Trash2, Download, Sparkles, Scan,
  ArrowUp, ArrowDown, CheckCircle, MapPin, FileText, Brain, ShieldAlert, Activity
} from 'lucide-react'
import {
  ResponsiveContainer, PieChart as RechartsPie, Pie, Cell,
  AreaChart, Area, XAxis, YAxis, Tooltip, RadarChart, PolarGrid, PolarAngleAxis, Radar
} from 'recharts'
  ...
  return (
    <motion.div className="relative w-11 h-11" whileHover={{ scale: 1.1 }}>
      ...
      <motion.div
        ...
        animate={{ boxShadow: ['0 0 12px rgba(139,92,246,0.4)', '0 0 24px rgba(139,92,246,0.6)', '0 0 12px rgba(139,92,246,0.4)'] }}
...
function StatCard({ icon: Icon, label, value, trend, variant = 'purple' }: any) {
  ...
  return (
    ...
    >
      ...
      <p className="text-xl font-bold">{typeof value === 'number' ? count : count}</p>
    </motion.div>
  )
}

function ThreatGauge({ level, confidence }: { level: string; confidence: number }) {
  const colors: any = {
    CRITICAL: '#ef4444',
    HIGH: '#f97316',
    MEDIUM: '#eab308',
  ...
  return (
    <div className="relative w-40 h-24 mx-auto">
      <svg viewBox="0 0 200 110" className="w-full">
        ...
        >
          <line x1="100" y1="95" x2="100" y2="45" stroke={color} strokeWidth="3" strokeLinecap="round" />
          <circle cx="100" cy="95" r="5" fill={color} />
        </motion.g>
      </svg>
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-center">
        <p className="text-lg font-bold" style={{ color }}>{Math.round(confidence * 100)}%</p>
        <p className="text-[9px] font-bold uppercase" style={{ color }}>{level}</p>
      </div>
    </div>
  )
}

function Btn({ children, onClick, disabled, loading, variant = 'primary', icon: Icon, className = '' }: any) {
  const variants: any = {
    primary: 'bg-purple-600 hover:bg-purple-500 text-white',
    secondary: 'bg-gray-800 border border-gray-700 hover:border-purple-500/50 text-gray-300',
  }
...
function IntelCard({ type, value, onCopy }: any) {
  const configs: any = {
    phone: { icon: Phone, color: 'text-cyan-400 bg-cyan-500/10 border-cyan-500/20' },
    upi: { icon: CreditCard, color: 'text-purple-400 bg-purple-500/10 border-purple-500/20' },
    link: { icon: Link2, color: 'text-red-400 bg-red-500/10 border-red-500/20' },
    email: { icon: Mail, color: 'text-yellow-400 bg-yellow-500/10 border-yellow-500/20' },
    aadhaar: { icon: Fingerprint, color: 'text-orange-400 bg-orange-500/10 border-orange-500/20' },
    keyword: { icon: AlertTriangle, color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' },
  }
...
function ChatBubble({ sender, text, index }: any) {
  ...
  return (
    ...
    >
      <div className={`p-2.5 rounded-lg ${isScammer ? 'bg-red-500/10 border-l-2 border-red-500 mr-4' : 'bg-green-500/10 border-l-2 border-green-500 ml-4'}`}>
        <div className="flex items-center gap-1 mb-1">
          {isScammer ? <UserX size={10} className="text-red-400" /> : <Bot size={10} className="text-green-400" />}
          <span className={`text-[9px] font-bold uppercase ${isScammer ? 'text-red-400' : 'text-green-400'}`}>
...
function PhishingScanner({ links }: { links: string[] }) {
  ...
  return (
    <div className="mt-3 p-3 rounded-xl bg-red-500/10 border border-red-500/20">
      ...
      {links.map((link, i) => (
        <div key={i} className="text-[10px] p-1.5 rounded bg-black/20 mb-1 flex items-center gap-2">
          <Link2 size={10} className="text-red-400" />
          <span className="font-mono truncate flex-1">{link}</span>
          <span className="px-1.5 py-0.5 rounded bg-red-500/30 text-red-300">DANGEROUS</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="mt-3 p-3 rounded-xl bg-orange-500/10 border border-orange-500/20">
      ...
      {phones.slice(0, 3).map((phone, i) => (
        <div key={i} className="flex items-center justify-between text-[10px] p-1.5 rounded bg-black/20 mb-1">
          <span className="font-mono">{phone}</span>
          <span className="px-1.5 py-0.5 rounded bg-yellow-500/30 text-yellow-300">SUSPICIOUS</span>
        </div>
      ))}
    </div>
  )
...
function ScammerPrediction({ category }: { category: string }) {
  const predictions: any = {
    BANKING_FRAUD: ['Will ask for OTP', 'May request screen share', 'Will create urgency'],
  ...
  const preds = predictions[category] || ['Will extract personal info', 'Will create false urgency']
  ...
  return (
    <div className="mt-3 p-3 rounded-xl bg-purple-500/10 border border-purple-500/20">
      ...
      {preds.map((pred: string, i: number) => (
        <div key={i} className="flex items-center gap-1.5 text-[10px] text-gray-300 mb-1">
...
// ============================================
const EXAMPLES = [
  { id: 'bank', label: 'Banking Fraud', icon: CreditCard, text: 'URGENT! SBI account BLOCKED! Call +91-9876543210, share OTP! Acc: 12345678901234' },
  { id: 'upi', label: 'UPI Fraud', icon: Zap, text: '₹15,000 cashback! Send ₹99 to verify. UPI: cashback@ybl. Call: 8765432109' },
  { id: 'lottery', label: 'Lottery Scam', icon: Sparkles, text: 'Won ₹25 Lakhs! Pay ₹5,000 to lottery@paytm. Call: +91-7654321098' },
  { id: 'kyc', label: 'KYC Fraud', icon: FileText, text: 'Paytm suspended! http://paytm-kyc.xyz Share Aadhaar+OTP. Call: 9123456780' },
  { id: 'job', label: 'Job Scam', icon: Target, text: 'Work from home ₹50K! Pay ₹1,000 registration. Call: +91-8899776655' },
  { id: 'crypto', label: 'Crypto Scam', icon: TrendingUp, text: 'DOUBLE money! 200% returns! http://crypto-profit.ml UPI: invest@ybl' },
]
...
export default function Dashboard() {
  ...
  const checkConnection = useCallback(async () => {
    try {
      const res = await fetch(`${API_URL}/api/health`, { headers: { 'x-api-key': API_KEY } })
      setConnected(res.ok)
    } catch {
      setConnected(false)
    }
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [analyticsRes, sessionsRes, intelRes] = await Promise.all([
        fetch(`${API_URL}/api/analytics/dashboard`, { headers: { 'x-api-key': API_KEY } }),
        fetch(`${API_URL}/api/sessions`, { headers: { 'x-api-key': API_KEY } }),
        fetch(`${API_URL}/api/intelligence`, { headers: { 'x-api-key': API_KEY } }),
      ])
      if (analyticsRes.ok) setAnalytics(await analyticsRes.json())
      if (sessionsRes.ok) setSessions((await sessionsRes.json()).sessions || [])
      if (intelRes.ok) setIntelligence((await intelRes.json()).intelligence || [])
    } catch (e) {
      console.error(e)
    }
  }, [connected])
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/honeypot`, {
        ...
        body: JSON.stringify({
          sessionId,
          message: { sender: 'scammer', text: currentMessage, timestamp: Date.now() },
      ...
      if (!res.ok) {
        ...
        setToast(`Error ${res.status}: ${errData.detail || errData.message || 'Request failed'}`)
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="font-bold text-lg">
                SCAM<span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-cyan-400">SHIELD</span>
              </h1>
              <p className="text-[9px] text-gray-500">AI Honeypot • v4.0</p>
            </div>
          </div>
          <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs ${connected ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-red-500/10 border-red-500/30 text-red-400'}`}>
      ...
      <main className="relative z-10 max-w-[1600px] mx-auto px-4 py-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
          <StatCard icon={Radio} label="Sessions" value={analytics?.realtime?.activeSessions || 0} trend={12} variant="purple" />
          <StatCard icon={Target} label="Scams" value={analytics?.realtime?.scamsDetectedToday || 0} trend={8} variant="red" />
          <StatCard icon={Database} label="Intel" value={analytics?.realtime?.intelligenceExtracted || 0} trend={25} variant="cyan" />
          <StatCard icon={Zap} label="Response" value={analytics?.realtime?.avgResponseTime || '1.2s'} variant="green" />
        </div>
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {EXAMPLES.map((ex) => (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-medium">{ex.label}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl bg-gray-900/60 border border-gray-800">
                  ...
                  <div className="flex gap-2 mt-2">
                    <Btn onClick={sendMessage} disabled={!message.trim() || !connected} loading={loading} icon={Send} className="flex-1">
                      Analyze
                    </Btn>
                    <Btn onClick={clearSession} variant="secondary" icon={Trash2} />
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl bg-gray-900/60 border border-gray-800">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-[8px] text-gray-500 font-mono">{sessionId.slice(0, 10)}...</span>
                  </div>
                  <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                    {conversation.length === 0 ? (
                      <p className="text-center py-6 text-gray-500 text-xs">Send message to start</p>
                    ) : (
                      conversation.map((msg, i) => <ChatBubble key={i} sender={msg.sender} text={msg.text} index={i} />)
                    )}
                  ...
                  <>
                    ...
                    <div className="p-4 rounded-xl bg-gray-900/60 border border-gray-800">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <Btn onClick={generateReport} variant="secondary" icon={Download} className="text-[10px] px-2 py-1">
                          Report
                        </Btn>
                      </div>
                      <ThreatGauge level={result.analysis?.threatLevel || 'SAFE'} confidence={result.analysis?.confidenceScore || 0} />
                      <div className="grid grid-cols-2 gap-2 mt-3">
                        <div className="p-2 rounded-lg bg-gray-800/50 text-center">
                          <p className="text-[9px] text-gray-500">Category</p>
                          <p className="text-xs font-semibold">{result.analysis?.scamCategory?.replace(/_/g, ' ') || 'N/A'}</p>
                        </div>
                        <div className="p-2 rounded-lg bg-gray-800/50 text-center">
                          <p className="text-[9px] text-gray-500">Agent</p>
                          <p className="text-xs font-semibold">{result.agentState?.personaName || 'N/A'}</p>
                        </div>
                      </div>
                      {result.analysis?.detectedKeywords?.length > 0 && (
                        <div className="mt-2">
                          <p className="text-[9px] text-gray-500 mb-1">Keywords</p>
                          <div className="flex flex-wrap gap-1">
                            {result.analysis.detectedKeywords.slice(0, 6).map((kw: string, i: number) => (
                              <span key={i} className="px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400 text-[9px]">
                                {kw}
                              </span>
                            ))}
                          </div>
                    ...
                    <ScammerPrediction category={result.analysis?.scamCategory} />
                    <PhishingScanner links={result.extractedIntelligence?.phishingLinks || []} />
                    <PhoneReputation phones={result.extractedIntelligence?.phoneNumbers || []} />
                  </>
                ) : (
                ...
                <div className="p-4 rounded-xl bg-gray-900/60 border border-gray-800">
                  ...
                  {result?.extractedIntelligence ? (
                    <div className="space-y-1.5 max-h-44 overflow-y-auto">
                      {Object.entries(result.extractedIntelligence).map(([key, values]: any) =>
                        values.map((value: string, i: number) => (
                          <IntelCard
                            key={`${key}-${i}`}
                            type={key.replace('Numbers', '').replace('Ids', '').replace('Links', '').replace('Addresses', '').toLowerCase()}
                            value={value}
                            onCopy={() => copyToClipboard(value)}
                          />
                        ))
                      )}
                      {Object.values(result.extractedIntelligence).flat().length === 0 && (
                        <p className="text-center py-4 text-gray-500 text-xs">No intelligence</p>
                      )}
                    </div>
                  ) : (
                    <p className="text-center py-4 text-gray-500 text-xs">Intel will appear here</p>
                  )}
                </div>
                ...
                <div className="h-48">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData}>
                      ...
                      <Area type="monotone" dataKey="scams" stroke="#a855f7" fill="url(#colorArea)" strokeWidth={2} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-gray-900/60 border border-gray-800">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-3 rounded-lg bg-purple-500/10 border border-purple-500/20 text-center">
                    ...
                    <p className="text-lg font-bold text-purple-400">{analytics?.totals?.totalSessions || 0}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-center">
                    <p className="text-[10px] text-gray-400">Scams</p>
                    <p className="text-lg font-bold text-red-400">{analytics?.totals?.totalScamsDetected || 0}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-center">
                    ...
                    <p className="text-lg font-bold text-cyan-400">{analytics?.totals?.totalIntelligence || 0}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/20 text-center">
                    <p className="text-[10px] text-gray-400">Success</p>
                    <p className="text-lg font-bold text-green-400">{analytics?.totals?.successRate || '0%'}</p>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          {activeTab === 'sessions' && (
            <motion.div key="sessions" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="p-4 rounded-xl bg-gray-900/60 border border-gray-800">
                ...
                {sessions.length === 0 ? (
                  <p className="text-center py-8 text-gray-500 text-xs">No sessions yet</p>
                ) : (
                  <div className="space-y-1.5">
                    {sessions.map((session, i) => (
                      <div key={i} className="flex items-center justify-between p-2.5 rounded-lg bg-gray-800/50 border border-gray-700">
                        <div>
                          <p className="text-xs font-mono">#{session.sessionId?.slice(0, 12)}...</p>
                          <p className="text-[10px] text-gray-500">
                            {session.scamCategory?.replace(/_/g, ' ') || 'Unknown'} • {session.messageCount} msgs
                          </p>
                        </div>
                        <span
          ...
          {activeTab === 'intel' && (
            <motion.div key="intel" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="p-4 rounded-xl bg-gray-900/60 border border-gray-800">
                <h3 className="text-sm font-semibold mb-3">Intelligence ({intelligence.length})</h3>
                {intelligence.length === 0 ? (
                  <p className="text-center py-8 text-gray-500 text-xs">No intelligence yet</p>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1.5">
                    {intelligence.slice(0, 30).map((item, i) => (
                      <IntelCard key={i} type={item.type} value={item.value} onCopy={() => copyToClipboard(item.value)} />
                    ))}
                  </div>
                )}
              </div>
